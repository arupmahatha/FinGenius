...
class StockAnalyzer:
    ...
    def _get_human_input(self, prompt: str, default_value=None) -> str:
        ...
        try:
            ...
            with col1:
                if st.button("Accept Default", key=f"accept_{prompt[:20]}"):
                    st.write("Using default value")
                    return default_value
            with col2:
                if st.button("Submit Modified Value", key=f"submit_{prompt[:20]}"):
                    st.write(f"Using modified value: {user_input}")
                    return user_input
            
            # Return default if no button was clicked
            return default_value
...
class StockAnalyzer:
    ...
    def _get_human_input(self, prompt: str, default_value=None) -> str:
        ...
        try:
            ...
            st.write(prompt)
            user_input = st.text_input("Enter your response or press Enter to accept default:", value=default_value)
            if st.button("Accept Default"):
                st.write("Using default value")
                return default_value
            elif st.button("Submit Modified Value"):
                st.write(f"Using modified value: {user_input}")
                return user_input
            return default_value
        except:
